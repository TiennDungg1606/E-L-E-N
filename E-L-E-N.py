...
# Hàm gửi dữ liệu giọng nói lên server
def send_audio_to_server(audio_file_path):
    url = "http://localhost:5000/recognize"  # Địa chỉ server của bạn
    with open(audio_file_path, 'rb') as audio_file:
        response = requests.post(url, files={'file': audio_file})
    return response.json()

# Gọi hàm này khi bạn cần gửi âm thanh lên server và nhận phản hồi
response_data = send_audio_to_server('path_to_audio_file.wav')
...
# Hàm phát hiện ngôn ngữ của người dùng
...
# Hàm dịch câu nói của người dùng sang tiếng Anh
def translate_text(text, target_lang='en'):
    try:
        translator = Translator()
        translation = translator.translate(text, dest=target_lang)
        return translation.text  # Trả về văn bản đã dịch
    except Exception as e:
        print(f"Translation error: {e}")
        return text  # Trả lại văn bản gốc nếu gặp lỗi
...
def record(clearChat=True, iconDisplay=True):
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            # Nếu ngôn ngữ không phải tiếng Anh, dịch câu nói sang tiếng Anh
            if language != 'en':
                said = translate_text(said, 'en')
                print(f"Translated to English: {said}")
...
class ELENApp:
    def __init__(self, root):
        self.root = root
        self.is_running = True  # Trạng thái điều khiển vòng lặp
        
        # Bind sự kiện minimize và restore cửa sổ
        self.root.bind("<Unmap>", self.on_minimize)  # Khi cửa sổ bị minimize
        self.root.bind("<Map>", self.on_restore)     # Khi cửa sổ được restore

        # Chạy voiceMedium trong một thread riêng để không chặn giao diện
        self.thread = threading.Thread(target=self.voiceMedium)
        self.thread.daemon = True  # Để thread tự động dừng khi chương trình thoát
        self.thread.start()

    def on_minimize(self, event):
        """Dừng hoạt động khi cửa sổ bị minimize."""
        self.is_running = False

    def on_restore(self, event):
        """Tiếp tục hoạt động khi cửa sổ được mở lại."""
        self.is_running = True

    def voiceMedium(self):
        speak("Hello Sir. How can I help you?", True, True)
        while True:
            if self.is_running:
                query = record()
                ...
                if isContain(query, EXIT_COMMANDS):
                    ...
                    main(query.lower())
            time.sleep(0.1)  # Thêm sleep để giảm tải CPU
        os.system('taskkill /im E-L-E-N.exe')
...
def keyboardInput(e):
    ...
    if user_input != "":
        ...
        if isContain(user_input, EXIT_COMMANDS):
            ...
            # Nếu ngôn ngữ không phải tiếng Anh, dịch câu lệnh sang tiếng Anh
            if language != 'en':
                user_input = translate_text(user_input, 'en')
                print(f"Translated to English: {user_input}")
...
def main(text):
		...
		if isContain(text, ['shutting down', 'shut down']):
			os.system("shutdown /s /t 9")
			speak("Your computer will be turned off after 5 seconds. See you again!", True, True)
			os.system('taskkill /im E-L-E-N.exe')
			return

		if isContain(text, ['restart', 'rebot','reset']):
			os.system("shutdown /r /t 9")
			speak("Your computer will be restarted after 5 seconds. See you again!", True, True)
			os.system('taskkill /im E-L-E-N.exe')
			return
		...
		if isContain(text, ['weather']):
			location = text
			location = text.replace('what is the weather in', '').replace('what is the weather in', '').replace('what is weather in', '').replace('what is the weather', '').replace('what the weather', '').replace('the weather in', '').replace('the weather', '').replace('weather in', '').replace('weather', '').replace('today', '').strip()

			if location == "":
				location = None
			data = webScrapping.weather(location)
			speak('', False, True)
		...
		if result != "None": speak(result, True, True)
			...
			speak("Sorry. I couldn't understand your query... ", True, True)
...
def showSingleImage(type, data=None):
	...
	if type=="weather":
		...
		image_label = Label(weather, image=img4, bg=chatBgColor)
		image_label.pack()

		img_width = img4.width()
		
		temp_label = Label(weather, text=data[1], font=('Montserrat', 15), bg=chatBgColor)
		temp_label.pack()  # Hiển thị tạm thời để đo chiều rộng
		temp_label.update_idletasks()  # Cập nhật để lấy chiều rộng chính xác
		text_width = temp_label.winfo_width()  # Chiều rộng của văn bản
		temp_label.pack_forget()  # Ẩn Label tạm thời
		
		x_position = (img_width - text_width) // 2
		
		Label(weather, text=data[0], font=('Arial Bold', 40), fg='white', bg='#3F48CC').place(x=65,y=45)
		Label(weather, text=data[1], font=('Montserrat', 15), fg='white', bg='#3F48CC').place(x=x_position, y=110)
		Label(weather, text=data[2], font=('Montserrat', 10), fg='white', bg='#3F48CC').place(x=65,y=140)
		Label(weather, text=data[3], font=('Arial Bold', 12), fg='white', bg='#3F48CC').place(x=65,y=160)
...
if __name__ == '__main__':
	...
	image = Image.open("extrafiles/images/E-L-E-N_2.png")
	photo = ImageTk.PhotoImage(image)	
	splash_label = Label(splash_root, image=photo, bg='#000000')
	splash_label1 = Label(splash_root, text="Processing E-L-E-N...", font=('montserrat',15),bg='#000000',fg='white')
	
	splash_label.pack(pady=0.5)
	splash_label1.pack(pady=1)
	...
	botIcon = PhotoImage(file="extrafiles/images/E-L-E-N1.png")
	...
	try:
		# pass
		Thread(target=ELENApp(root).start())
	except:
		pass
	try:
		# pass
	...
	root.iconbitmap('extrafiles/images/E-L-E-N.ico')
	raise_frame(root1)
	root.mainloop()